...
const missingPath = path.join(repoRoot, "missing-relations.txt");
...
function sync() {
  ...
  // 截取两端
  const prefix = indexRaw.slice(0, startIdx + startMarker.length);
  const suffix = indexRaw.slice(endIdx);
  ...
  // 合并已有自动补充区中已经定义的条目（保留它们）
  // 我们将在 startMarker 后插入一个注释说明，然后追加所有 unique 条目
  const header = `\n  // 自动同步自 missing-relations.txt，共 ${unique.length} 条新条目（去重后）\n`;
  const entries = unique.map(buildEntry).join("\n") + "\n";

  const newContent = prefix + header + entries + "\n  " + suffix;
